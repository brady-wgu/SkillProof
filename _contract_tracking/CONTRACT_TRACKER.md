...
| SOW-2.5 | SOW §2.5 Development | **Student Web Experience** (LTI auth, chat, AI scoring, feedback, error/retry), **AI Engine** (multi-LLM, routing, prompts, token/cost/latency, fallback, guardrails), **Admin Portal** (tenant controls, modules, instructors, analytics, reporting, audit logs), **Data & Integration Services** (LTI 1.3, API export, logging) | Feature | Yes | Student portal · Tenant Admin portal (config + modules + analytics on `tenant-19` + audit on `tenant-20`) · Instructor portal · Super Admin portal (instructors consolidated on `super-12` by documented WGU deviation 13 May 2026) | In Dev | JFT | All four user-type portals deployed | Spine of the SkillProof build. Replaces MVP-only framing. v4.51: tenant-level analytics + audit gaps closed; instructor management consciously consolidated under Super Admin. |
...
| SOW-2.7 | SOW §2.7 Deployment | Environment provisioning, CI/CD, staging+production deploys, logging/monitoring/alerting, LMS-side LTI setup support, deployment walkthrough | Process | Yes | tenant-07 (Deploy stepper) · super-02 (monitoring) | In Dev | JFT | Production deploy + release notes + deployment checklist | |
| SOW-2.8 | SOW §2.8 Support | Pilot support: monitoring, critical/high issue resolution, AI debugging, fixes/patches, analytics/logs support | Process | Yes | tenant-09..tenant-16 (SC-ADD-06 SLA flow) | In Dev | JFT | Support SLAs met; weekly/biweekly status updates delivered | |
...
| SOW-9.1 | SOW §9.1 Support Overview | Tech support, issue resolution, operational assistance; WGU business hours; Jira ticketing | Process | Yes | tenant-12, tenant-13 (Jira ticket creation) | In Dev | JFT | Jira project provisioned; SLAs published | |
...
| SOW-9.4 | SOW §9.4 Support Channels | Email · Jira · Video calls · Slack/Teams (optional) · CSM | Process | Yes | tenant-12..tenant-14 | In Dev | JFT | Channels operational | |
| SOW-9.5 | SOW §9.5 SLAs | P1 <2hr / 4-8hr · P2 <4hr / 24hr · P3 <1 business day / 72hr · P4 <2 business days / next release | Process | Yes | tenant-14 (CSM 2hr response thread) · tenant-16 (SLA dashboard) | In Dev | JFT | SLAs published; first incidents meet targets | |
...
| SOW-13 | SOW §13 Change Management | Jira / email submission; impact analysis; written approval before implementation; tracked to closure | Process | Yes | tenant-12..tenant-14 (Jira workflow illustrated) | Active | Both | Change requests documented via Jira | |
...
|:---|:---|:---|:---|:---|:---|:---|:---|
| A-6.1 | Multi-LLM providers (Claude, ChatGPT, Gemini, others) — orchestration layer | Yes | tenant-05 model picker · super-10 external tooling · throughout student coaching loop | In Dev | JFT | Provider switching demonstrated in UAT | LLM-agnostic orchestration. |
| A-6.2 | Route requests to different LLMs by use case / configuration | Yes | tenant-05 model picker · super-10 | In Dev | JFT | Routing policies enforced and audited | |
...
| A-6.5 | Fallback mechanisms for LLM service interruptions | Yes | tenant-10, tenant-11 (fallback engaged) · tenant-15 (service restored) | In Dev | JFT | Fallback engages within seconds; demonstrated in SC-ADD-06 | |
...
| A-6.7 | Configurable standard guardrails on all prompts for all LLMs | Yes | tenant-06 (4 short text-box guardrails) · referenced in student coaching | In Dev | JFT | Centralized guardrails injected into every prompt | |
| A-6.8 | A/B testing framework for LLM configurations | Yes | tenant-06 ("A/B test variants" badge on Configure AI Coaching Prompt) | In Design | JFT | A/B experiments with statistical reporting | Partial coverage: medium-fidelity badge declares the capability; full A/B test config + results surface is a future enhancement. v4.45 re-audit downgraded Gap → Partial. |
...
| A-6.24 | Automated CI/CD pipeline | Yes | tenant-07 (5-step CI/CD Stepper) · super-10 GitHub Actions | In Dev | JFT | Blue/green deploys via GitHub Actions | |
...
| A-7.6 | Multi-language support capability | Yes | tenant-17 (Branding — default locale dropdown) | In Dev | JFT | Localization framework in place; English-only at pilot | English-only at pilot per SOW Assumption. |
| A-7.7 | Progressive Web App (PWA) functionality | Yes | tenant-17 (PWA install badge) | Not Started | JFT | Installable PWA on student portal | v4.18 added the badge; functionality TBD. |
...
| A-7.9 | Customizable interface for institutions | Yes | tenant-17 (Branding & Customization) | In Dev | JFT | Logos, colors, domain configurable per tenant | |
| A-7.10 | Comprehensive student engagement tracking | Yes | tenant-19 (Engagement section: time-on-task chart + submissions trend) · instructor-02..instructor-08 · student-08 progress map · student-11 welcome-back returning-user | In Dev | JFT | Engagement dashboards live for educators | v4.51 closed Tenant Admin coverage gap. FERPA-bounded per A-10.1. v4.58 renumbered student-06 → student-08 (progress map) and added student-11 (welcome back) after live-MVP rebuild. |
| A-7.11 | Learning analytics dashboard for educators | Yes | tenant-19 (Class Insights section: subject summary + at-risk badges + drill-down) · instructor-02..instructor-08 | In Dev | JFT | Class-wide insights + at-risk flagging + drill-down | v4.51 closed Tenant Admin coverage gap. |
| A-7.12 | Usage statistics and reporting | Yes | tenant-19 (Program Reports section: filters + Engagement / Submissions / Topic-Mastery reports) · super-03 token usage · instructor-02 KPIs | In Dev | JFT | Usage reports filterable + exportable | v4.51 closed Tenant Admin coverage gap. |
| A-7.13 | Data visualization tools | Yes | tenant-19 (accessible SVG charts in Engagement section) · instructor-03 heatmap · super-02 KPI gauges · super-04 cost chart | In Dev | JFT | Charts accessible (screen reader, hover context) | v4.51 added Tenant Admin coverage. |
| A-7.14 | Export capabilities for all reports | Yes | tenant-19 (Program Reports: CSV / PDF / JSON per report row) · instructor-03 PDF/CSV CTAs | In Dev | JFT | CSV / PDF / JSON / XML one-click exports | v4.51 closed Tenant Admin coverage gap. XML added per v4.18 (§16.3 #8.9 alignment). |
...
| ID | Requirement | Commitment | Storyboard Coverage | Build Status | Owner | Acceptance | Notes |
|:---|:---|:---|:---|:---|:---|:---|:---|
| A-8.1 | Full LTI 1.3 (including Advantage services) compliance for student authentication | Yes | student-01 (post-LTI entry: Coding Coach landing) · tenant-08 (LRPS launch URL) · lrps-01 | In Dev | JFT | SSO + grade return + deep linking + names/roles provisioning | Pilot uses Basic LTI 1.3 only; Advantage features are post-pilot per WGU direction. v4.58 student-01 was the zyBooks LTI launch illustration; after the live-MVP rebuild, student-01 is the first post-LTI screen (the Coding Coach landing the user lands on after the LRPS deep link). The LTI 1.3 launch itself is non-visual; coverage on this row stays valid. |
...
| A-8.3 | Deep linking support | Yes | tenant-08 (LRPS deep-link provisioning) · lrps-01 | In Dev | JFT | LTI Advantage deep linking operational | Future-state: per-sub-section deep links. |
...
| ID | Requirement | Commitment | Storyboard Coverage | Build Status | Owner | Acceptance | Notes |
|:---|:---|:---|:---|:---|:---|:---|:---|
| A-9.1 | 24/7 system monitoring | Yes | super-02 (active alerts) · tenant-09 (all systems operational) | In Dev | JFT | 24x7 cloud + application monitoring operational | |
| A-9.2 | 99.95% uptime SLA | Yes | tenant-16 (SLA dashboard) · super-02 | In Dev | JFT | Quarterly SLA report meets 99.95% | ~4.4 hours downtime budget per year. |
...
| A-9.7 | Dedicated customer success manager | Yes | tenant-14 (CSM Jordan in chat thread) | Active | JFT | CSM Jordan assigned per §9.7 | |
| A-9.8 | Regular software updates and feature enhancements | Yes | Non-visual (process) | Active | JFT | Maintenance cycle published | |
| A-9.9 | Bug fix SLA: Critical 4hr / High 24hr / Medium 72hr | Yes | tenant-16 SLA dashboard | In Dev | JFT | Bug fix SLA reports meet targets | |
| A-9.10 | 24/7 technical support with <2hr response for critical | Yes | tenant-14 (P1 SLA <2hr) | In Dev | JFT | First-incident response <2hr | |
...
| A-9.13 | Proactive monitoring, alerting, issue resolution | Yes | super-02 (alerts) · tenant-09 (baseline) · tenant-10 (degradation) | In Dev | JFT | Alerts route to on-call engineer | |
| A-9.14 | Self-service support portal | Yes | help-01 (Help & Resources surface — self-service section: search, 3 cards, updates feed, Contact JFT Support button) · tenant-* navbar Help link (v4.51) | In Design | JFT | Self-service portal scoped + delivered | v4.45 added the surface in a new shared `help/` folder; v4.51 added the navbar Help link on every tenant_admin screen. |
| A-9.15 | Video training resources | Yes | help-01 (Help & Resources surface — role-filtered video gallery with 9 tiles + featured callout) · tenant-* navbar Help link (v4.51) | In Design | JFT | Video modules per audience in support library | v4.45 added the surface; v4.51 added the navbar Help link from every tenant_admin screen. |
...
| A-10.2 | Data retention and deletion policies compliant with FERPA | Yes | tenant-18 (Subject Lifecycle & Archival) · super-06 | In Dev | JFT | 3-year default retention; coordinated deletion across primaries/backups | Cross-references MSA-10. |
...
| A-10.4 | Audit logging for all data access and modifications | Yes | tenant-20 (tenant-scoped Activity Log) · super-08 (cross-tenant audit log) · instructor-08 (audit trail) · tenant-18 (audit ref) | In Dev | JFT | Immutable audit logs for read/write actions | v4.51 added tenant-scoped view. |
...
| A-10.8 | Role-based access control (RBAC) | Yes | super-09 (Access Control: People / Skills / Schools tabs) · super-12 (read-only Roster) · super-13 (read-only School listing) · tenant-01 (elevation) · super-14 / tenant-21 / instructor-09 (Access Denied) | In Dev | JFT | Granular RBAC across UI + API; logged approvals; deny screen surfaces role mismatch; many:one Skill↔School and Admin↔School assignments edited in one place per RBAC v1.0 §7. | v4.57 added Access Denied screens per RBAC doc v1.0 §4.2 Figure 6. v4.68 consolidated permission writes onto super-09 (Access Control); super-12 and super-13 became read-only browse views. |
