...
| `super-09` | Access Control (4-tier role taxonomy + Skill / School many:one assignments) | SC-ADD-04 ext | A-10.8 RBAC · SOW-2.5 (instructors/admins management) · RBAC v1.0 §7 (Super Admin sole controller of access levels, Skill ownership, Tenant assignment) · RBAC v1.0 §5 (Skill duplication across Schools) | A-10.4 audit on elevation + permission changes | Contract-required | Keep | v4.27 expanded to Student/Instructor/Tenant Admin/Super Admin; v4.29 LTI baseline annotations. v4.68 renamed User Management → Access Control; absorbed permission-write surface from `super-12`/`super-13`; tabbed People · Skills · Schools with inline chip-cluster many:one editor. |
...
| `super-12` | Instructor Roster (cross-tenant, read-only coverage view as of v4.68) | SC-ADD-04 ext | SOW-2.5 · A-10.8 RBAC · A-10.4 audit | A-10.13 platform access governance | Contract-required | Keep | v4.48: moved from tenant_admin (prior `tenant-22`) under WGU direction that Super Admin is the sole controller of platform access. Cross-tenant scope; per-tenant filter at the top. v4.52: tenant labels normalized to WGU Schools; Skill-assignment authority callout added. v4.68: demoted to read-only browse view — Manage/Reactivate buttons removed; assignment edits route to `super-09` Access Control · Skills tab so writes have one source of truth. |
| `super-13` | School / Tenant Management (read-only listing + Create new School as of v4.68) | SC-ADD-04 ext | A-8.6 multi-tenancy isolation · A-10.8 RBAC · SOW-2.5 Admin Portal | A-10.4 audit on School creation / deactivation | Contract-required | Keep | v4.52: Super Admin manages the 4 WGU Schools as tenants. List + per-row Manage + top-of-page `Create new School`. Grounding: Brady's RBAC doc v1.0 (13 May 2026) — "tenants = WGU Schools." v4.68: per-row Manage buttons removed; Tenant Admin assignments routed to `super-09` Access Control · Schools tab. `Create new School` form preserved here (creation flow stays on this screen). |
