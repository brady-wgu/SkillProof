...
| **SC-ADD-05** | **Data Portability.** Data & APIs landing (REST + GraphQL APIs + real-time webhooks per §16.3 #8.12) → REST/GraphQL API console (method + endpoint + query params, auth/scoping side panel, rate-limit display) → sample JSON response (syntax-highlighted code block) → one-click export wizard (engagement metrics dataset) → format picker (CSV / JSON / PDF / XML tab switcher with 5-row preview, per §16.2 #7.14 + §16.3 #8.9) → download confirmation with file metadata + audit ID. | 6 | 10–15 |
...
- **Tab switcher** (segmented control) for CSV / JSON / PDF / XML format selection (per SOW §16.2 #7.14 + §16.3 #8.9)
